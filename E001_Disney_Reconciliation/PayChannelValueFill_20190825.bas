...
Sub SearchThenFill()
    ...
    payChannel = Array("网站销售渠道", "M站销售渠道", "APP", "Box Office", "DSA", "Club 33", _
                        ...
                        "Tongcheng", "Tuzuu", "Maiqin", "Fumubang", "IPH", "Damai", "大麦直销渠道",  _
                        "SMG live", "Yongle", "Yuanjing", "Klook", "HuiXiang", "Qunar", "Motianlun", _
                        "Fulfillment", "DRC-Hotel Bundle","Maoyan")
    ...
    '相对位置第39行
    ActiveCell.Offset(39, 0).Range("A1").Value = payChannelValue(34)
    ...
    '=====================================================================================================
    '填入单元格"GATickets"（M站 + 网站）,相对位置第41行
    If payTypeValue(0) <> "" And payTypeValue(1) <> "" Then
        ActiveCell.Offset(41, 0).Range("A1").Value = payTypeValue(0) + payTypeValue(1)
    ElseIf payTypeValue(0) <> "" Then
        ActiveCell.Offset(41, 0).Range("A1").Value = payTypeValue(0)
    ElseIf payTypeValue(1) <> "" Then
        ActiveCell.Offset(41, 0).Range("A1").Value = payTypeValue(1)
    End If

    '填入单元格"Box Office",相对位置第50行
    ActiveCell.Offset(50, 0).Range("A1").Value = payTypeValue(2)

    '填入单元格"DRC"，相对位置第56行
    ActiveCell.Offset(56, 0).Range("A1").Value = payTypeValue(3)
    '=====================================================================================================


    '检查现金部分中是否已有订单存在退款，如有，则将退款值填充进相应区域(单元格："其他调整项"），相对位置第51行
    ActiveCell.Offset(51, 0).Range("A1").Value = deductionValue
    '=====================================================================================================
